...
test("server-renders complete beginner installation documentation", async () => {
  ...
  assert.match(html, /Windows · Terminal/);
  assert.match(html, /Windows · PowerShell/);
  assert.match(html, /Windows · Git Bash/);
  assert.match(html, /Windows · Command Prompt/);
  assert.match(html, /git clone https:\/\/github\.com\/Angadslr\/Token-Optimizer\.git/);
  assert.match(html, /\.venv\/bin\/python -m pip install -e/);
  assert.match(html, /\.\/\.venv\/Scripts\/python\.exe -m pip install -e/);
  assert.match(html, /cd Token-Optimizer/);
  assert.match(html, /python -m venv \.venv/);
  assert.match(html, /If <code>python<\/code> is not recognized, use <code>python3<\/code>/);
  assert.match(html, /You do not need to switch shells/);
  assert.match(html, /\$env:NVIDIA_API_KEY = (?:"|&quot;)paste-your-key-here(?:"|&quot;)/);
  assert.match(html, /export NVIDIA_API_KEY=(?:"|&quot;)paste-your-key-here(?:"|&quot;)/);
  assert.match(html, /set NVIDIA_API_KEY=paste-your-key-here/);
  assert.match(html, /%USERPROFILE%/);
  assert.doesNotMatch(html, /Set-Location Token-Optimizer/);
  assert.doesNotMatch(html, /py -3 --version/);
  assert.doesNotMatch(html, /py -3 -m venv/);
